...
## 2026-09-26 — نصب موفق WordPress پایه در محیط آزمایشی

### اجرای واقعی
پس از آماده‌سازی Subdomain و دیتابیس مستقل، WordPress فارسی در محیط آزمایشی نصب و اتصال آن به دیتابیس با موفقیت تأیید شد.

- آدرس محیط تست: `https://test.karafarinak.ir`
- مسیر اجرا: `/domains/test.karafarinak.ir/public_html`
- PHP محیط تست: **8.1.34**
- نسخه WordPress نصب‌شده: **7.1 فارسی (fa_IR)**
- Database: `h423580_wp_test`
- Database Host: `localhost`
- Database User: `h423580_wp_test`
- رمز دیتابیس: **در گزارش/GitHub ثبت نشده است.**
- نام کاربری مدیر WordPress: `karafarinak_admin`
- ایمیل مدیر WordPress: `info@karafarinak.ir`
- درخواست عدم ایندکس توسط موتورهای جستجو برای محیط تست در زمان نصب فعال شد.

### آماده‌سازی فایل‌ها
- بسته رسمی `wordpress-7.1-fa_IR.zip` دریافت و در Document Root آپلود شد.
- فایل ZIP استخراج شد و محتویات آن مستقیماً به `public_html` منتقل شد.
- پوشه موقت `wordpress` و فایل ZIP پس از انتقال حذف شدند.
- فایل‌های هسته WordPress مانند `wp-admin`, `wp-content`, `wp-includes` و `index.php` در Document Root قرار دارند.
- فایل موقت `phpinfo.php` که برای بررسی نسخه PHP استفاده شده بود حذف شد.

### نتیجه تست اولیه
صفحه نصب WordPress بدون خطا باز شد و اتصال WordPress به دیتابیس تأیید گردید؛ سپس نصب فارسی با موفقیت کامل شد و صفحه «وردپرس فارسی نصب شد» نمایش داده شد.

### وضعیت فعلی پروژه
- WordPress پایه: **نصب و تأیید شد.**
- WooCommerce: **هنوز نصب نشده.**
- افزونه‌های جانبی: **هنوز نصب نشده‌اند.**
- Site2/طراحی نهایی: **هنوز وارد مرحله پیاده‌سازی WordPress نشده است.**
- سایت اصلی `karafarinak.ir`: **بدون تغییر.**
- دیتابیس `h423580_payment`: **بدون تغییر و خارج از محیط Test.**

### گام بعدی قفل‌شده
1. ورود به پیشخوان WordPress و بررسی سلامت WordPress خام.
2. انجام تست پایه در محیط Test.
3. پس از تأیید WordPress خام، نصب WooCommerce.
4. تست WooCommerce قبل از نصب افزونه‌های جانبی.
5. سپس نصب افزونه‌ها به‌صورت یکی‌یکی و تست پس از هر مرحله.
6. در مراحل بعدی، پیاده‌سازی نسخه تأییدشده Site2 و مقایسه بصری با Previewهای مرجع انجام خواهد شد.

**اصل کنترل ریسک همچنان برقرار است: محیط Test مستقل است و هیچ تغییر مستقیمی روی سایت اصلی یا دیتابیس Live انجام نشده است.**


## 2026-09-26 — به‌روزرسانی تست WooCommerce در محیط آزمایشی

### وضعیت WordPress و WooCommerce
پس از نصب WordPress پایه، نسخه WordPress به **7.1.2** به‌روزرسانی شد و بررسی Health Check انجام گرفت.

سپس WooCommerce رسمی توسط Automattic نصب و فعال شد:
- WooCommerce: **11.1.2**
- WordPress: **7.1.2**
- PHP: **8.1.34**
- MySQL: **8.0.46**
...
- PHP max upload / post size: **256 MB**
- HPOS (Custom Order Tables): فعال
- صفحات Store، Cart، Checkout و My Account توسط WooCommerce ایجاد و شناسایی شدند.
- افزونه‌های جانبی پیشنهادی در Wizard مانند PayPal، Jetpack، Pinterest، MailPoet، Google Listings & Ads و Snapchat نصب/فعال نشدند.
- استفاده از داده‌های usage tracking ووکامرس نیز فعال نشد.
- محیط همچنان صرفاً **Test** است و به سایت اصلی متصل نشده است.

### تست عملی فروشگاه
برای اطمینان از عملکرد پایه WooCommerce:
1. یک محصول آزمایشی ساده ایجاد و منتشر شد.
2. صفحه محصول با موفقیت باز شد.
3. گزینه «افزودن به سبد خرید» با موفقیت اجرا شد.
4. سبد خرید محصول را دریافت کرد.

بنابراین عملکرد پایه Product → Add to Cart → Cart در محیط آزمایشی تأیید شد.

### موارد قابل پیگیری
در گزارش System Status ووکامرس، درخواست wp_remote_post() با خطای **403** ثبت شده است. این مورد هنوز اصلاح نشده و باید قبل از اتصال سرویس‌های خارجی/درگاه‌ها بررسی شود؛ فعلاً هیچ تغییر تنظیماتی برای رفع آن اعمال نشده است.

PHP نسخه **8.1.34** نیز در Health Check قدیمی تشخیص داده شده، اما برای جلوگیری از تغییر هم‌زمان چند متغیر، فعلاً نسخه PHP تغییر داده نشده است.

موارد غیرفعال مانند **Akismet** و **Hello Dolly** نیز فعلاً فعال نشده‌اند و در مرحله پاک‌سازی محیط تست قابل حذف هستند.

### وضعیت مرحله‌ای فعلی
- Subdomain و SSL: **تأیید شد**
- Database مستقل Test: **تأیید شد**
- WordPress پایه: **نصب و به‌روزرسانی شد**
- WooCommerce: **نصب و فعال شد**
- تست ایجاد محصول: **موفق**
- تست افزودن به سبد خرید: **موفق**
- پرداخت واقعی: **هنوز انجام نشده و در این مرحله لازم نیست**
- افزونه‌های جانبی: **هنوز نصب نشده‌اند**
- پیاده‌سازی Site2 روی WordPress: **هنوز شروع نشده**
- سایت اصلی و دیتابیس Live: **بدون تغییر**

### گام بعدی
قبل از نصب افزونه‌های بعدی، خطای wp_remote_post() با وضعیت **403** بررسی می‌شود. پس از مشخص شدن علت و اطمینان از سلامت ارتباطات خروجی، افزونه‌های موردنیاز به‌صورت یکی‌یکی نصب و بعد از هر نصب تست خواهند شد.


## 2026-09-26 — معماری دسترسی، تست و انتشار برای ادامه توسعه کارآفرینک

برای کاهش کارهای دستی، جلوگیری از تغییر مستقیم سایت اصلی و فراهم‌کردن امکان توسعه سریع‌تر در آینده، ساختار زیر به‌عنوان الگوی عملیاتی پروژه ثبت شد:

### تفکیک محیط‌ها و منابع
1. **Source of Truth طراحی**
   - Repository: `samadkarami-boop/KARAFARINAK-WEB-Demos`
   - مسیر: `Site2/`
   - هر تغییر طراحی ابتدا در Site2 انجام و نسخه آن ثبت می‌شود.

2. **محیط آزمایشی WordPress**
   - URL: `https://test.karafarinak.ir`
   - محیط مناسب برای نصب/تست WordPress، WooCommerce، قالب و افزونه‌ها.
   - تغییرات آزمایشی نباید مستقیماً به Live منتقل شوند.

3. **Production**
   - URL: `https://karafarinak.ir`
   - فقط نسخه تأییدشده پس از تست کامل باید به این محیط منتقل شود.
   - دسترسی عملیاتی به Production باید حداقلی و محافظت‌شده باشد.

4. **GitHub**
   - مرجع کد، نسخه‌ها، گزارش‌ها و تاریخچه تغییرات.
   - GitHub جایگزین محیط اجرای WordPress نیست.

### سیاست دسترسی
- برای محیط Test یک حساب FTP محدود ساخته شد:
  - Account: `testsite@karafarinak.ir`
  - Root: `/home/h423580/domains/test.karafarinak.ir/public_html/`
  - دسترسی به کل Home یا Production ندارد.
- رمز FTP عمداً در GitHub و گزارش ثبت نمی‌شود.
- حساب اصلی FTP `h423580` برای عملیات عادی استفاده نشود.
- در صورت فراهم‌شدن ابزار اتصال مستقیم معتبر در آینده، اولویت با همین حساب محدود Test یا یک حساب محدودتر و موقت خواهد بود؛ دسترسی مستقیم به Production بدون ضرورت ایجاد نشود.

### مدل عملیاتی پیشنهادی آینده
`GitHub → Test → تست مرورگر/عملکرد → تأیید → Release → Production`

هر تغییر مهم باید:
1. در منبع مشخص ثبت شود.
2. روی Test اجرا شود.
3. عملکرد و ظاهر آن بررسی شود.
4. نتیجه در گزارش ثبت شود.
5. فقط نسخه تأییدشده برای انتشار آماده شود.

این ساختار برای قالب‌های بعدی، به‌روزرسانی‌های WooCommerce، افزونه‌ها، فرم‌ها، پیامک، درگاه و تغییرات دوره‌ای سایت نیز قابل استفاده است.

## 2026-09-26 — بررسی گزینه‌های دسترسی مستقیم برای آینده

هدف این بررسی، کاهش رفت‌وبرگشت‌های دستی در زمان توسعه و نگهداری است؛ نه ایجاد دسترسی دائمی و بدون کنترل به سایت اصلی.

### گزینه‌های قابل استفاده
**1. اتصال مستقیم WordPress**
یک گزینه موجود در اکوسیستم ابزارهای ChatGPT با نام **WPVibe** امکان اتصال به WordPress خودمیزبان را ارائه می‌کند و برای مدیریت محتوا، بررسی افزونه/قالب، رسانه و برخی عملیات مدیریتی WordPress طراحی شده است. این مسیر برای کارهای داخل WordPress از FTP مناسب‌تر است، چون به‌جای کار با فایل خام، با خود WordPress کار می‌کند.

**2. دسترسی به یک کامپیوتر کنترل‌شده**
ابزاری با عنوان **Remote Desktop Commander** برای اتصال به یک کامپیوتر مجاز و کنترل فایل‌سیستم و Terminal آن ارائه شده است. در صورت استفاده از یک PC/VPS کنترل‌شده که روی آن FileZilla یا ابزار SSH نصب باشد، این معماری می‌تواند در آینده واسط امن‌تری برای عملیات فایل/ترمینال باشد؛ البته فقط پس از اتصال و تأیید کاربر.

**3. FTP مستقیم**
در بررسی فعلی، اتصال مستقیم FTP به ParsPack به‌عنوان یک Connector آماده در دسترس پیدا نشد. بنابراین حساب FTP فعلی ساخته شده ولی هنوز اتصال مستقیم آن به ChatGPT ایجاد نشده است.

### تصمیم فعلی
- فعلاً هیچ اتصال جدیدی به Production ایجاد نشود.
- حساب FTP محدود Test حفظ شود.
- تا مشخص‌شدن نتیجه تیکت 403 و وضعیت سرویس، تنظیمات سرور تغییر نکند.
- برای آینده، ابتدا اتصال مستقیم WordPress روی Test بررسی شود؛ سپس در صورت نیاز، مسیر Remote Desktop/FTP/SSH برای عملیات فایل بررسی شود.
- هر دسترسی جدید باید حداقل سطح لازم را داشته باشد و ترجیحاً فقط به Test محدود شود.

## 2026-09-26 — نتیجه پشتیبانی درباره خطای wp_remote_post()

پشتیبانی ParsPack اعلام کرد طبق بررسی آن‌ها، خطای `wp_remote_post()` به محدودیت اتصال سرویس WordPress از هاست‌های داخل ایران مربوط است و در هاست ایران امکان اتصال به سرورهای WordPress و در نتیجه استفاده از `wp_remote_post()` و توابع مشابه ممکن است وجود نداشته باشد.

پشتیبانی دو مسیر را مطرح کرد:
1. انتقال هاست به اروپا برای بررسی/رفع محدودیت اتصال.
2. انتقال هاست به یک Node دیگر در ایران؛ با این توضیح که تغییر Node با تغییر IP همراه است.

### تصمیم پروژه
- تغییر Node ایران فعلاً تأیید نشد.
- ابتدا درباره امکان انتقال فقط محیط Test به اروپا و تأثیر آن بر سرویس اصلی سؤال شد.
- تا دریافت پاسخ تکمیلی، هیچ تغییر Node یا مهاجرتی انجام نمی‌شود.
- خطای `wp_remote_post()` در محیط Test به‌عنوان یک محدودیت زیرساختی مستندسازی شد و نباید بدون دلیل به خرابی WordPress یا WooCommerce نسبت داده شود.

### وضعیت
- WordPress/WooCommerce Test: عملیاتی.
- مسیر پایه فروشگاه: تست شده.
- `wp_remote_post()`: خطای 403؛ علت اعلام‌شده از سوی پشتیبانی، محدودیت هاست ایران.
- FTP محدود Test: ساخته و آماده استفاده در صورت نیاز.
- Production: بدون تغییر.
- پاسخ تکمیلی درباره امکان محیط Test اروپا: در انتظار.
